...
public class RunningVariance extends GroupProcessor
{
	public RunningVariance()
	{
		...
		try
		{
			...
			FunctionProcessor square = new FunctionProcessor(new FunctionTree(Numbers.power, new ArgumentPlaceholder(0), new Constant(2)));
			Connector.connect(sm_1, OUTPUT, square, INPUT);
			FunctionProcessor minus = new FunctionProcessor(Numbers.subtraction);
